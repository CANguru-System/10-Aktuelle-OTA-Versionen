...
const uint16_t adr_offTime = lastAdr0 + 0x01;
// Kanalnummer #2 - Helligkeit
const uint16_t adr_onTime = lastAdr0 + 0x03;
// Kanalnummer #3 - Dim
const uint16_t adr_Dim = lastAdr0 + 0x05;
// Kanalnummer #24- Geschwindigkeit
const uint8_t adr_speed = lastAdr0 + 0x07;
// Kanalnummer #5 ff- LEDProgramm für die 4 Kanäle festlegen
const uint16_t adr_prog = lastAdr0 + 0x08; // ab dieser Adresse werden die Weichenstellungen gespeichert
const uint16_t lastAdr = adr_prog + 1;
...
void sendConfig()
{
  ...
  uint8_t arrKanal00[NumLinesKanal00] = {
      /*1*/ Kanal00, numberofKanals, 0, 0, 0, 0, 0, decoderadr,
