...
void proc_fromWDP2CAN()
{
  ...
  if (packetSize)
  {
    ...
    case S88_Polling:
      ...
      break;
    case S88_EVENT:
      // Original zuerst retten
      memcpy(&CANbuffer, &UDPbuffer, CAN_FRAME_SIZE);
      // Abfrage auf jeden Fall bedienen, auch wenn es keinen solchen Kontakt gibt
      UDPbuffer[0x01]++;
      UDPbuffer[0x09] = 0x00; // is free
      UDPbuffer[0x04] = 8;
      sendToWDPfromCAN(UDPbuffer);
      // Original an die Rückmelder senden
      // dann kann die tatsächliche Belegung von dort gemeldet werden
      proc2Clnts(CANbuffer, toClnt);
      break;
    }
  }
}
...
void loop()
{
  ...
  case startGleisbox:
    ...
    // Empfangen über ESPNOW werden registriert
    espInit();
    delay(200);
