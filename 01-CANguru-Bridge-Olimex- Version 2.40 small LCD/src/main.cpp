...
{
  /*  uint8_t ChipRevision = getChipRevision();
    Serial.println("Chip Revision: " + String(ChipRevision));
    if (ChipRevision >= 2)
    {
      Serial.println("CAN-Speed doubled: 500");
      ESP32Can.CANInit(GPIO_NUM_5, GPIO_NUM_35, ESP32CAN_SPEED_500KBPS);
    }
    else
    {
      Serial.println("CAN-Speed: 250");
...
void proc_fromUDP2CAN()
{
  ...
  if (packetSize)
  {
    ...
    log_e("fromUDP2CAN: %X", UDPbuffer[0x01]);
...
void proc_fromCAN2SYSandGW()
{
  ...
  if (ESP32Can.CANReadFrame(&MessageReceived) == ESP32CAN_OK)
  {
    ...
    // now dispatch
    log_e("proc_fromCAN2SYSandGW: %X", UDPbuffer[0x01]);
    switch (UDPbuffer[0x01])
    {
    case PING: // PING
...
void analyseTCP(uint8_t *buffer)
{
  ...
  log_e("analyseTCP: %X", buffer[0x01]);
  switch (buffer[0x01])
  {
  case Lok_Speed:
  case Lok_Direction:
